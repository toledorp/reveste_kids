...
//segredo para gerar tokem da API
...
// Função para autenticar(logiM) um usuario
const loginUser = async(req, res) =>{
    try{
        const { email, password} = req.body
        //primeira validação: checar se email existe
        if (email != undefined){
            // Buscandfo o usuario no bd
            ...
            if (user != undefined) {
                ...
                if (correct) {
                    ...
                    res.status(401).json({error: "suas credenciais invalidas. Tente novamente"})
        ...
        console.log(error)
            res.status(500).json({error: 'Não foi possivel realizar o login. Erro interno do sistema'})
        }
    }
